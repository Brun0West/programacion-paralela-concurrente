...
int main() {
    ...
    #pragma omp parallel
    {
        ...
        // Reducción: sumar locales a global (evita race con critical)
        #pragma omp critical
        for (int b = 0; b < bin_count; b++) {
            bin_counts[b] += local_bin_counts[b];
        }
